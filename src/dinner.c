#include "../includes/philosophers.h"

static void *ft_lone_philo(void *arg)
{
    t_philo *philo;

    philo = (t_philo *)arg;
    wait_all_threads(philo->arguments);
    set_long(&philo->philo_mutex, &philo->last_meal_time, gettime(MILLISECOND));
    ft_increase_long(&philo->arguments->arguments_mutex, &philo->arguments->threads_running_nbr);
    write_status(TAKE_FIRST_FORK, philo, DEBUG_MODE);
    while (!simualte_finished(philo->arguments))
        usleep(200);
    return (NULL);
}


/*
* eatint routin
* 1 -> grabing forks 🍴 -> LOCK
* 2 -> Eating 🍽️
* 3 -> Take the forks back -> UNLOCK
*/
static void eat (t_philo *philo)
{
    long i; 

    safe_mutex_handel(&philo->first_fork->fork, LOCK);
    write_status(TAKE_FIRST_FORK, philo, DEBUG_MODE);
    safe_mutex_handel(&philo->second_fork->fork, LOCK);
    write_status(TAKE_SECOND_FORK, philo, DEBUG_MODE);

    set_long(&philo->philo_mutex, &philo->last_meal_time, gettime(MILLISECOND));
    i = philo->meals_counter + 1;
    set_long(&philo->philo_mutex, &philo->meals_counter, i );
    write_status(EATING, philo, DEBUG_MODE);
    precise_usleep(philo->arguments->l_eat_time, philo->arguments);
    if (philo->arguments->l_eat_must > 0 
        && philo->meals_counter == philo->arguments->l_eat_must)
        set_bool(&philo->philo_mutex, &philo->full, true);
    
    safe_mutex_handel(&philo->first_fork->fork, UNLOCK);
    safe_mutex_handel(&philo->second_fork->fork, UNLOCK);
}


void thinking(t_philo *philo, bool pre_simulation)
{
    long	t_eat;
	long	t_sleep;
	long	t_think;

	if (!pre_simulation)
		write_status(THINKING, philo, DEBUG_MODE);
	if (philo->arguments->l_nbr_philosopher % 2 == 0)
		return ;
	t_eat = philo->arguments->l_eat_time;
	t_sleep = philo->arguments->l_sleep_time;
	t_think = (t_eat * 2) - t_sleep;
	if (t_think < 0)
		t_think = 0;
	precise_usleep(t_think * 0.42, philo->arguments);
}


void *dinner_simulation(void *data)
{
    t_philo *philo;

    philo = (t_philo *)data; 
    wait_all_threads(philo->arguments);
    set_long(&philo->philo_mutex, &philo->last_meal_time, gettime(MILLISECOND));

    ft_increase_long(&philo->arguments->arguments_mutex, &philo->arguments->threads_running_nbr);
    ft_desyncronized_philos(philo);

    while (!simualte_finished(philo->arguments))
    {
        if (philo->full)
            break;
        // eat 
        eat(philo);
        // sleep 
        write_status(SLEEPING, philo, DEBUG_MODE);
        precise_usleep(philo->arguments->l_sleep_time, philo->arguments);
        thinking(philo, false);

        // thinking
    }
     

    return (NULL);

}
/*
* 0) If not meals, return ->[0]
* 1) If only one philo -> ad hoc function 
* 2) Each philo need a thread 
* 3) Use a monitor thread 
* 4) Synconisis the stradrt to all philo threads 
* 5) JOIN everyone 
*/

void dinner_start (t_arguments *argument)
{
    int i_count;

    i_count = -1;

    printf ("%ld max meals %ld nbr of ph\n", argument->l_eat_must,  argument->l_nbr_philosopher );

    if (0 == argument->l_eat_must)
        return ;
    else if (1 == argument->l_nbr_philosopher)
        safe_thread_handel(&argument->philos[0].thread_id, ft_lone_philo, &argument->philos[0], CREATE);
    else 
    {
        while (++i_count < argument->l_nbr_philosopher)
        {
            safe_thread_handel(&argument->philos[i_count].thread_id, dinner_simulation,
                &argument->philos[i_count], CREATE);
            printf("created philo %i\n", i_count);
        }
    }
    safe_thread_handel(&argument->monitor_check_die, ft_monitor_dinner, argument, CREATE);
    argument->l_start_simulation = gettime(MILLISECOND);
    //printf("%ld start time\n\n", argument->l_start_simulation);
    set_bool(&argument ->arguments_mutex, &argument->all_threads_ready, true);

    // wait for everyone 
    i_count = -1;
    while (++i_count < argument->l_nbr_philosopher)
    {
        safe_thread_handel(&argument->philos[i_count].thread_id, NULL, NULL, JOIN);
        printf("joined Philo %i\n", i_count);
    }
    set_bool(&argument->arguments_mutex, &argument->l_end_simulation, true);
    safe_thread_handel(&argument->monitor_check_die, NULL, NULL, JOIN);
}